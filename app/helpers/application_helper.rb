module ApplicationHelper
  ...
  def login_links
    ...
    if !user_loggedin?
      links_string += link_to('LOGIN', login_path)
      links_string = links_string + ' | ' + link_to('REGISTER', signup_path)
    else
      icon = '<i class="material-icons">note_add</i>'.html_safe
      links_string += link_to(icon, new_article_path, class: 'orange_link')
      links_string += link_to(current_user.name, user_path(current_user))
      links_string = links_string + ' | ' + link_to('log out', logout_path)
    end
